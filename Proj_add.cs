...
namespace WindowsFormsApp1
{
    public partial class Proj_add : Form
    {
        ...
        private void button_add_Click(object sender, EventArgs e)
        {
            ...
            DataTable projects = db.ExecuteSql($"select * from projects where name = '{name}'");

            if(projects.Rows.Count > 0)
            {
                MessageBox.Show("Вот надо по-разному называть, ай-яй-яй", "Так нельзяяяяяяяяяяяяяяяяяяяяяяяя!");
            }
